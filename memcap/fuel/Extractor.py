...
class ListExtractor(object):
        ...
        for ind in thisRandIndx:
            ...
            for imgExt in self.dataExt:
                ...
                if os.path.isfile(thispath):
                    ...
                    break
            if  valid:    
                if  get_img and self.destin_shape is not None:
                    shape = tuple(self.destin_shape) + (self.TrainingMatinfo['datainfo']['channel'],)
    ...
    def single_get_parse_anno(self, thisname):
    ...
    def _get_parse_anno(self, thisname):
        ...
        thispath = os.path.join(self.annodir, thisfile)
        with open(thispath) as data_file:    
            anno_dict_list = json.load(data_file)
...
def get_cnn_img(thispath, shape, norm=True):
    ''' 
    shape should be in (row, col, channel) format.    
    '''
    img = imread(thispath)
